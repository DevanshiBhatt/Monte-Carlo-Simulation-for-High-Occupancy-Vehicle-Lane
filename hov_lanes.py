...
class Lanes:
    ...
    def fn_compute_avgspeed(self):
        ...
        for index, row in df.iterrows():
            if (row['weather'] == 'Winter' or row['weather'] == 'Rains') and row['weather_int'] > 3 \
                ...
                speed = format((np.median(self.rand_gen_pert(35, 45, 75, samples=10))), ".2f")
                gpv_speed =format((np.median(self.rand_gen_pert(25, 35, 50, samples=10))), '.2f')
    ...
    def fn_compute_avgtime():
        ...
        :return: None
        """
        df['hov_time'] = round(20.0/df['hov_speed (mph)'], 2)
        df['gpv_time'] = round(20.0/df['gpv_speed (mph)'], 2)
...
if __name__ == '__main__':
    ...
    my_lane.fn_fine()
    my_lane.fn_camera_functional(no_of_samples)
    my_lane.fn_compute_avgspeed()
    my_lane.fn_compute_avgtime()
    my_lane.fn_compute_pollution()
